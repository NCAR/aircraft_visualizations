...
import { setVisibleChartCount, setMapLayerVisibility, addChartVariable, removeChartVariable, moveChartVariableAxis, setChartAxisLabel, clearChartConfig } from '../../store/actions/uiActions.js';
...
export default class SettingsOverlay extends IComponent {
  ...
  setupPlotStylingControls() {
    ...
    if (clearBtn) {
      clearBtn.addEventListener('click', () => {
        ...
        this.dispatch(clearChartConfig(chartIndex));
      });
    }
    ...
    if (leftLabelInput) {
      leftLabelInput.addEventListener('change', () => {
        const state = this.getState();
        const chartIndex = getSelectedChartIndex(state);
        this.dispatch(setChartAxisLabel(chartIndex, 'left', leftLabelInput.value));
      });
    }
    if (rightLabelInput) {
      rightLabelInput.addEventListener('change', () => {
        const state = this.getState();
        const chartIndex = getSelectedChartIndex(state);
        this.dispatch(setChartAxisLabel(chartIndex, 'right', rightLabelInput.value));
      });
    }
  ...
  renderAvailableVariablesTable(state) {
    ...
    const allVars = getVariables(state) || [];
    const chartIndex = getSelectedChartIndex(state);
    ...
    container.querySelectorAll('.var-add-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (key) {
          ...
          this.dispatch(addChartVariable(chartIdx, key, 'left'));
  ...
  attachVariableListHandlers(container, chartIndex) {
    ...
    container.querySelectorAll('.axis-toggle-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        if (key && axis) {
          this.dispatch(moveChartVariableAxis(chartIndex, key, axis));
        }
      });
    });
    ...
    container.querySelectorAll('.variable-remove-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        const key = btn.getAttribute('data-key');
        if (key) {
          this.dispatch(removeChartVariable(chartIndex, key));
        }
      });
    });
  }
